 ...
 * time cursor; the heart diagram and the trace both subscribe and update
 * imperatively, so cause and effect stay frame-perfectly in sync and React
 * never re-renders 60×/second.
 *
 * Time is the source of truth in a ref; play/pause/speed are mirrored to React
 * state only so the controls can render their current value.
 */
...
export interface CardiacClock {
  ...
  /** Seek to a fraction 0..1 of the loop (used by the scrubber). */
  seekFraction: (f: number) => void
  getTime: () => number
  getFraction: () => number
  /** Register a per-frame callback; returns an unsubscribe fn. Fires once immediately. */
  subscribe: (fn: FrameListener) => () => void
}

const DEFAULT_SPEED = 0.4 // slowed-down for teaching; 1.0 = real time

export interface ClockOptions {
  /** Start time as a fraction 0..1 of the loop (for deep links / sharing). */
  startFraction?: number
  /** Whether to begin playing (default true). */
  autoplay?: boolean
}
...
export function useCardiacClock(durationMs: number, opts?: ClockOptions): CardiacClock {
  ...
  // New condition → new loop length. Honor the start fraction on first mount;
  // reset to the top only when the duration actually changes. Keyed on the
  // duration value (not a "first run" flag) so React StrictMode's double-invoke
  // in dev can't clobber the deep-linked start time.
  useEffect(() => {
  ...
  const subscribe = useCallback((fn: FrameListener) => {
    ...
    fn(timeRef.current) // immediate sync so a paused view paints right away
    return () => {
  ...
  return useMemo(
    () => ({
      ...
      seekFraction, getTime, getFraction, subscribe,
    }),
    [isPlaying, speed, durationMs, play, pause, toggle, setSpeed, seekFraction, getTime, getFraction, subscribe],
  )
}
