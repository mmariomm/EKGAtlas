 ...
 *
 * Anatomical frame (all leads + the heart vector live here):
 *   x = patient's LEFT      (+)
 *   y = INFERIOR / caudal   (+)   (downward, toward the feet)
 *   z = ANTERIOR / forward  (+)   (toward the chest wall)
 *
 * The frontal plane is (x, y) — this is what the six limb leads "see" and what
 * the clinical QRS *axis* describes. The transverse plane is (x, z) — what the
 * six precordial leads see.
 */
...
/** A smooth bell curve, peaking at 1.0 at `center`. `width` is the std-dev in ms. */
export const gaussian = (t: number, center: number, width: number): number =>
...
/**
 * Frontal-plane angle of a vector, in degrees, using the clinical hexaxial
...
/** Frontal-plane magnitude (length of the arrow drawn on the heart diagram). */
