...
/* ── real-world imperfections: PTB-XL noise annotations, normalized ────── */
const cleanNoise = (v) => {
  if (!v) return undefined
  const t = String(v).replace(/^[\s,]+|[\s,]+$/g, '').trim()
  return t || undefined
}
const noiseOf = (row) => {
  const n = {}
  const d = cleanNoise(row.baseline_drift)
  const s = cleanNoise(row.static_noise)
  const b = cleanNoise(row.burst_noise)
  const e = cleanNoise(row.electrodes_problems)
  if (d) n.d = d
  if (s) n.s = s
  if (b) n.b = b
  if (e) n.e = e
...
const GRIT = (target) => Math.floor(target / 3) // deliberately imperfect share
...
for (const cardId of ORDER) {
  ...
  const mine = Object.values(bank.strips).filter((s) => s.cardId === cardId)
  const already = mine.length
  const gritHave = mine.filter((s) => s.noise).length
  const gritWant = GRIT(target)
  // Two quotas: the class target (clean-preferred), plus a deliberate share of
  // imperfect real-world strips — noisy signal, same diagnostic certainty.
  const gritNeed = Math.max(0, gritWant - gritHave)
  if (already >= target && gritNeed === 0) { summary.push([cardId, already, 'kept']); continue }

  const all = db
    .filter((r) => r.validated_by_human === 'True' && matches(r, spec) && !usedEcg.has(Number(r.ecg_id)))
    .sort((a, b) => score(b, spec) - score(a, spec))
  const candidates = already >= target ? all.filter((r) => noiseOf(r)) : all
  ...
  for (const row of candidates) {
    const goal = target + (admitted >= target ? gritNeed : 0)
    if (admitted >= goal || tried >= (target + gritNeed) * 5) break
...
if (!DRY) {
  ...
  const lines = entries.map(([id, s]) =>
    `  { traceId: '${id}', cardId: '${s.cardId}', f: ${JSON.stringify(s.findings)}${s.noise ? `, n: ${JSON.stringify(s.noise)}` : ''} },`)
  const ts = `/**
...
/** Real-world imperfections annotated on the source record (lead ranges as
 * PTB-XL writes them; 'alles' = all leads): d drift, s static/muscle,
 * b burst artifact, e electrode problem. */
export interface QuizNoise { d?: string; s?: string; b?: string; e?: string }
export interface QuizStrip { traceId: string; cardId: string; f: QuizFindings; n?: QuizNoise }
